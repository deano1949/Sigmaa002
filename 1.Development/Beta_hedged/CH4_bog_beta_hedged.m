...
%Beta hedged
spy_pos=zeros(size(cl,1),1); %market index hedging position

%% Beta calculation 
load beta
beta1=backshift(1,beta);
beta1=[beta1 nan(2510,9)];%%%%%%-------------- temporary
Nanbeta=1;
%% ------move this to dataload
% spy_ret=mi.cl./backshift(1,mi.cl)-1; %SPY close price
% ret=cl./backshift(1,cl)-1;
% pt=91;
% beta=zeros(size(cl));
% while pt<=size(cl,1)
%     x=spy_ret(pt-90+1:pt);
%     for i=1:size(cl,2)
%         y=ret(pt-90+1:pt,i);
%         mld=ols(y,x);
%         beta(pt,i)=mld.beta;
...
for t=2:size(cl, 1)
 ...
 %%
    if strcmp(stckselectmode,'ranked')
        ...
        % beta neutral
        if ~isempty(hasData)
            hasBeta=beta1(t,hasData(idxSort(1:min(topN, length(idxSort)))));
            hasBeta(isnan(hasBeta))=1; %set NaN beta value to 1
            spy_pos(t)=sum(hasBeta);
        end
...
spytc=0.00013; % hedging cost
tc=TC_roundtrip*ones(size(positionTable));%trading cost estimate percentage
spyret=(mi.cl-mi.op)./mi.op; %spy return
hedgeret=-spyret.*spy_pos; hedgeret(hedgeret==0)=NaN;hedgeret=hedgeret-spytc;hedgeret(isnan(hedgeret))=0;% return of hedging
% hedgeret=zeros(size(hedgeret,1),1);
pnl=smartsum(positionTable.*(retO2C-tc), 2);
ret=(pnl+hedgeret)/topN;
ret(isnan(ret))=0;
...
save 'bog_performance.mat' bog_performance stockpick 
