function [bogret,sogret,bog_performance,sog_performance,bsog_table]=Dev_bogsog_stoplossonEarningdates(location,write,stckselectmode,spread_mode)
% additional analysis: Apply stop loss only on earning dates
%if it improves PNL
...
if strcmp(location,'Home')
    ...
    path='C:\Spectrion\Data\AllData\NewUniverse\';
...
if strcmp(tradeuniverse,'currentUniverse')
    currentUniverse=readtable(strcat(tradeuniversepath,'Active_Trading_universe_2017_05_08.csv'));
...
data.erndate=earndateloc(:,list); %adding earning dates data
...
[bogret,bog_performance, bogstockpick]=Dev_bog_stoplossonEarningdates(bogtopN,bogentryZscore,boglookback,stckselectmode,spread_mode,write,data,trade_earndate);
[sogret,sog_performance, sogstockpick]=Dev_sog_stoplossonEarningdates(sogtopN,sogentryZscore,soglookback,stckselectmode,spread_mode,write,data,trade_earndate);
...
ret_2017=bogsogret(3022:end); ytd_2017=prod(1+ret_2017)-1; sharpe_2017=mean(ret_2017)*sqrt(252)/std(ret_2017); mdd_2017=maxdrawdown(100*cumprod(1+ret_2017));
