function [ret,bog_performance, stockpick]=Dev_bog_stoplossonEarningdates(topN,entryZscore,lookback,stckselectmode,spread_mode,write,data,stoploss_on_earndate)
%BOG: Apply stop loss only on earning dates

%trade_earndate: 'Y' or 'N': if 'N', DO not trade over earning periods
...
stoploss_on_earndate='Y';
if strcmp(stoploss_on_earndate,'Y')
%% PNL on no earning dates
    earndate=data.erndate;
    positionTable1=positionTable;
    positionTable1(earndate==1)=0;
    retO2C=positionTable1.*((cl-op)./op);

%% PNL on earning dates with stoploss
    stopline=ma;
    exitprice= stopline.*(stopline>lo)+cl.*(stopline<=lo);
    positionTable2=positionTable;
    positionTable2(earndate==0)=0;
    retO2Cearningdate=positionTable2.*((exitprice-op)./op);

    retfinal=retO2C+retO2Cearningdate;    
else
    retfinal=(cl-op)./op;
end
...
pnl=smartsum(positionTable.*(retfinal-tc), 2);
ret=pnl/topN;
ret(isnan(ret))=0;
...
ret_2017=ret(3022:end); ytd_2017=prod(1+ret_2017)-1; sharpe_2017=mean(ret_2017)*sqrt(252)/std(ret_2017); mdd_2017=maxdrawdown(100*cumprod(1+ret_2017));
ret_2016=ret(2771:3022); ytd_2016=prod(1+ret_2016)-1; sharpe_2016=mean(ret_2016)*sqrt(252)/std(ret_2016); mdd_2016=maxdrawdown(100*cumprod(1+ret_2016));
...
bog_performance.(naming)=mat2dataset(output_table,'VarNames',{'APR','SharpeRatio','maxDrawdown'},'ObsNames',{'Since Inception','Y2017','Y2016','Y2015','Y2014','Y2013','Y2012','Y2011','Y2010','Y2009','Y2008','Y2007','Y2006'});
